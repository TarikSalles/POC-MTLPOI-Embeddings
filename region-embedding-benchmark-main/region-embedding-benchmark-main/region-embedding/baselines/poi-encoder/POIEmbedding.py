...
class PreProcess:
    ...
    def _align_by_intersection(self, reg_key: str):
      ...
      # 2) interseção: só regiões que aparecem em ambos
      poi_regs  = set(self.pois[reg_key].unique())
      bor_regs  = set(self.boroughs[reg_key].unique())
      regions   = sorted(poi_regs & bor_regs)

      # (opcional) se quiser também podar POIs que caíram fora:
      self.pois = self.pois[self.pois[reg_key].isin(regions)].reset_index(drop=True)

      # 3) reindexa boroughs exatamente nesta ordem
      self.boroughs = (self.boroughs
          .set_index(reg_key).loc[regions].reset_index())

      # 4) mapeia chave -> índice (0..R-1) e region_id consistente
      reg2idx = {r:i for i,r in enumerate(regions)}
      self.region_id = [reg2idx[r] for r in self.pois[reg_key].tolist()]

      # 5) a partir daqui, calcule TUDO usando self.boroughs já reindexado:
    ...
    def read_poi_data(self):
        ...
        right_cols = ["h3","geometry"] if self.h3 else ["GEOID","geometry"]
        self.pois = gpd.sjoin(
            self.pois[["feature_id","category","fclass","geometry"]],
            self.boroughs[right_cols],
            how="inner",
            predicate="intersects",
        ).reset_index(drop=True)

        # sanity check:
        need = "h3" if self.h3 else "GEOID"
        if need not in self.pois.columns:
            raise KeyError(f"Spatial join não trouxe '{need}'. Colunas: {list(self.pois.columns)}")
    ...
    def read_boroughs_data(self):
        ...
        print("boroughs cols:", list(self.boroughs.columns))
        print(self.boroughs.head())
        print(self.boroughs)
    ...
    def save_data(self):
        self.pois.to_csv('pois.csv', index=False)
        self.edges.to_csv('edges.csv', index=False)
...
class POI2Vec:
    def __init__(self):
        if os.path.exists("pois.csv") and os.path.exists("edges.csv"):
            self.pois = pd.read_csv("pois.csv")
            self.pois["geometry"] = self.pois["geometry"].apply(wkt.loads)
            self.pois = gpd.GeoDataFrame(self.pois, geometry="geometry", crs="EPSG:4326")
    ...
    def save_walks(self):
        pkl.dump(self.second_class_walks, open("second_class_walks.pkl", "wb"))
    
    def read_walks(self):
        self.second_class_walks = pkl.load(open("second_class_walks.pkl", "rb"))
